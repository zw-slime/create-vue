...
import type {Res} from "@/interface";
...
// 响应拦截器
request.interceptors.response.use((res: AxiosResponse<Res>) => {
    ...
    if ([ErrCode.TokenEmpty, ErrCode.TokenExpired, ErrCode.TokenInvalid].includes(data.errCode)) {
        auth.setToken('');
    ...
    if (data.errCode !== ErrCode.Success) {
        ...
        ElMessage.error(data.errorInfo || '未知错误，请联系管理员');
